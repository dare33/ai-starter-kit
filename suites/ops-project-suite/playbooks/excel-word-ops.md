...
Gotcha detail lives in `~/developer/lessons/excel.md` and
`~/developer/lessons/office-files.md` — this playbook is the workflow; read those when
something breaks.
...
3. **Normalise after programmatic edits:** `soffice --headless --convert-to xlsx` — raw
   openpyxl saves strip dropdowns/CF and can make Table-bearing files unopenable.
...
- **Declare sources read-only.** Anything returned by someone else (per-owner or per-unit
  ...
  Name the master and the read-only set in the project's KEY RULES.
...
- **Rebuild scripts must read-and-preserve user columns.** Never re-run a builder that
  clears a column the user has filled.
- **Confirm the target workbook** (source vs rollup vs master) before building anything.

## Merge / consolidation protocol

Two sanctioned modes — the owner picks per instruction. (A rule that forbids what actually
happens gets violated and breeds status drift — learned the hard way, when a
no-bulk-merges rule was on the books the same day a full bulk merge ran. Define the safe
version instead.)
...
- **Bulk mode (on explicit instruction, e.g. "merge all"):** allowed, with mandatory
  ...
  **any** stakes level; create the file on the first bulk instruction if absent);
  anomalies **flagged, never silently fixed**; everything bulk-merged is **review-pending**
  until the owner reviews (surface a review aid — approval column, exceptions tab).
- **In-flight state during a bulk run lives in the verification-log ONLY.** At bulk start,
  ...
  A resumed session reads the ledger tail for exactly where the run stopped (one fact,
  one home; the per-unit QA line already records each unit).
- Sequence either way: back up master → migrate only the agreed columns, matched
  line-by-line on a stable key → recalc → verify summaries → one changelog line with
  backup path and verification result.
- **Parallelize checks, never writes.** Per-unit QA checks are independent — fan them out
...
- Evidence to `supporting-evidence/` named `<who>-<what>-<n>.<ext>`; check returned docs
  for **embedded images** (screenshots don't come out as text — extract and transcribe).
- Roll-up design rules (AVERAGE vs SUM, single baseline cell, caps per line) →
